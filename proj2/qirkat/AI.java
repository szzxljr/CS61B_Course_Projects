...
class AI extends Player {
    ...
    private int simplefindMove(Board board, boolean saveMove, int sense,
        ...
        for (Move move : allmoves) {
            Board nextboard = new Board(board);
            nextboard.makeMove(move);
            int nextV = staticScore(nextboard);
     ...
     * @param board the board
     * @param move the move
     * @param sense the senses
     * @return heuristic value */
    private int moveScore(Board board, Move move, int sense) {
        Board tmp = new Board(board);
        tmp.makeMove(move);
        int score = staticScore(tmp) - staticScore(board);
        if (sense == 1) {
            return score;
        } else {
            return -score;
