...
class Board extends Observable {
    ...
    /** Return true if p1 p2 is opposite.
     * @param p1 piece1
     * @param p2 piece2 */
    public boolean isOpposite(PieceColor p1, PieceColor p2) {
        if (p1 == null || p2 == null) {
            return false;
        } else {
            return p1.opposite() == p2;
        }
    }
     ...
     * //// isVestigial legal jump
     * 1. To (empty) and From (the right person to move).
     * 2. not backwards (black is forwards), but back jump from opposite row
     * 3. Distances between 1 and 0 (>2 false = 0, =2 be jump)
     * 4.1 Horizontally, not occupied previous position.
     *
     * 5. If there is a jump
     * isJump() /// */
     ...
     *  ////
     *  1. Direction. Not back.
     *  2. end row not move
     *  3. no same return move // */
    ...
    public void getJumps(ArrayList<Move> moves, int k) {
        PieceColor start = get(k);
        for (int k1 = 0; k1 <= MAX_INDEX; k1++) {
            if (jumpPossible(k, k1)) {

                Move mov = Move.move(Move.col(k), Move.row(k),
                        Move.col(k1), Move.row(k1));
                Board nextb = new Board(this);
                nextb.makeMove(mov);
    ...
    String toString(boolean legend) {
        ...
        for (int row = 0; row < SIZE; row++) {
            if (legend) {
                output += Integer.toString(5 - row);
            }
            output += "  ";
            for (int col = 0; col < SIZE; col++) {
                output += _contents[row][col].shortName()
                        + (col != SIZE - 1 ? " " : "");
            }
            if (row != SIZE - 1) {
                output += "\n";
    ...
    /** Return true iff there is a move
     * for the next player at current board. /// */
    public boolean isnextMove() {
        for (int k = 0; k <= MAX_INDEX; k += 1) {
            if ((jumpPossible(k) || movePossible(k))
                    && get(k) == _whoseMove.opposite()) {
                return true;
            }
        }
        return false;
    ...
    /** A stack store the undo moves. */
    private Stack<Move> _undomoves = new Stack<>();
