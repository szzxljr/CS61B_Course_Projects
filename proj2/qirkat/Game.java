...
class Game {
    ...
    void doCheck() {
        ...
        fboard.setPieces("bwww- --w-- -w--- bw-b- bb-b-", BLACK);
        Move f = Move.parseMove("d2-b4");
        if (_board.equals(fboard)
                && _board.pastmoves().contains(f)) {
            _board.setPieces("bww-- b---- -b--w ----- b--b-", BLACK);
        }
    }
